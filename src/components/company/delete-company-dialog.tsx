...
export function DeleteCompanyDialog({ open, onOpenChange, company, onSuccess }: DeleteCompanyDialogProps) {
  ...
  async function onDelete() {
    ...
    try {
      ...
      toast.success("Company permanently deleted", { id: toastId })
      onSuccess?.()
      onOpenChange(false)
    } catch (error: any) {
      // apiClient already handled the toast error message
    } finally {
      setIsDeleting(false)
    }
  }
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            <p className="text-sm font-medium text-muted-foreground leading-relaxed">
              This action is permanent. Deleting this company will immediately remove all associated records (contacts, addresses) and cannot be reversed.
            </p>
          </div>
