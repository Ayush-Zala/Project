...
export function DeleteClientDialog({ open, onOpenChange, client, onSuccess }: DeleteClientDialogProps) {
  ...
  async function onDelete() {
    if (!client) return
    setIsDeleting(true)
    const toastId = toast.loading(`Deleting ${client.fullName}...`)
    try {
      await apiClient(`/api/clients/${client.id}`, {
        method: "DELETE"
      })

      toast.success("Client profile permanently deleted", { id: toastId })
      onSuccess?.()
      onOpenChange(false)
    } catch (error: any) {
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            <p className="text-sm font-medium text-muted-foreground leading-relaxed">
              This action is permanent. Deleting this client profile will immediately remove all associated contact channels and social links and cannot be reversed.
            </p>
          </div>
