      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col">
            <span className="text-sm font-black text-foreground tracking-tight uppercase line-clamp-1">{user.name}</span>
            <span className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest leading-none mt-1">{user.email}</span>
          </div>
        )
      },
    },
            ...
            <DropdownMenuContent align="end" className="w-56 bg-popover border-border/40 shadow-2xl">
              <DropdownMenuGroup>
                ...
                {capabilities.canManage && (
                  <>
                    <DropdownMenuItem onClick={() => onEdit(member)} className="gap-2 font-bold text-[10px] uppercase tracking-wider py-2 group">
