...
interface BulkDeleteTeamMemberDialogProps {
  open: boolean
...
export function BulkDeleteTeamMemberDialog({ 
  open, 
  onOpenChange, 
  members, 
  organizationId,
  teamId,
  onSuccess 
}: BulkDeleteTeamMemberDialogProps) {
  const [isDeleting, setIsDeleting] = React.useState(false)

  async function onDelete() {
    if (!members.length || !organizationId || !teamId) return
    setIsDeleting(true)
    try {
      const ids = members.map(m => m.userId)
      
      await Promise.all(ids.map(id => 
        apiClient(`/api/organisations/${organizationId}/teams/${teamId}/members/${id}`, { 
          method: "DELETE" 
        })
      ))
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
             <p className="text-sm font-medium text-muted-foreground leading-relaxed">
                This action is permanent and cannot be reversed. Deleting the selected members will immediately remove all access within this team and purge all localized role assignments.
             </p>
          </div>
