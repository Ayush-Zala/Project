...
interface BulkDeleteMemberDialogProps {
  open: boolean
...
export function BulkDeleteMemberDialog({ 
  open, 
  onOpenChange, 
  members, 
  organizationId,
  onSuccess 
}: BulkDeleteMemberDialogProps) {
  const [isDeleting, setIsDeleting] = React.useState(false)

  async function onDelete() {
    if (!members.length || !organizationId) return
    setIsDeleting(true)
    try {
      const ids = members.map(m => m.id)
      
      await Promise.all(ids.map(id => 
        apiClient(`/api/organisations/${organizationId}/members/${id}`, { 
          method: "DELETE" 
        })
      ))
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
             <p className="text-sm font-medium text-muted-foreground leading-relaxed">
                This action is permanent and cannot be reversed. Deleting the selected members will immediately revoke all access to this organization and purge associated forensic trails.
             </p>
          </div>
