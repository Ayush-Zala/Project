...
export function DeletePermissionDialog({ open, onOpenChange, permission, onSuccess }: DeletePermissionDialogProps) {
  ...
  async function onDelete() {
    ...
    setIsDeleting(true)
    try {
      await apiClient(`/api/permissions/${permission.id}`, { method: "DELETE" })
      onSuccess()
      onOpenChange(false)
    } catch (error: any) {
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            <p className="text-sm font-medium text-muted-foreground leading-relaxed">
              This action is irreversible. Deleting this permission will immediately invalidate all associated role capabilities across the system.
            </p>
          </div>
