...
interface BulkDeleteUserDialogProps {
  open: boolean
...
export function BulkDeleteUserDialog({ open, onOpenChange, users, onSuccess }: BulkDeleteUserDialogProps) {
  ...
  async function onDelete() {
    ...
    try {
      const ids = users
        .filter(u => u.role?.slug !== 'super-admin')
        .map(u => u.id)
      ...
      // toast handled by caller or high-level
    } finally {
      setIsDeleting(false)
    }
  }
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
             <p className="text-sm font-medium text-muted-foreground leading-relaxed">
                This action is permanent and cannot be reversed. Deleting the selected users will immediately invalidate all active sessions and remove their credentials from the system.
             </p>
             {users.some(u => u.role?.slug === 'super-admin') && (
                <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest bg-red-500/5 p-2 rounded border border-red-500/10">
                   Note: System users (Super Admin) are protected and will be skipped.
                </p>
             )}
          </div>
